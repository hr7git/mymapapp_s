...
def main():
    ...
    if section == "글로벌 Top 기업 주가":
        ...
        if chart_type == "라인 차트":
            ...
            fig.update_layout(
                title=f"선택 기업 주가 추이 - {selected_period}",
                xaxis_title="날짜",
                yaxis_title="주가 (USD)",
                hovermode='x unified',
                height=600,
    ...
    # -----------------------------------------------------------
        ...
        selected_aw_period = st.sidebar.selectbox(
            "기간 선택 (올웨더 자산):",
        ...
        with st.spinner("올웨더 자산 데이터를 불러오는 중..."):
            ...
            for asset_name, ticker in ALL_WEATHER_ASSETS.items():
                data = get_stock_data(ticker, aw_period_options[selected_aw_period])
                if data is not None and not data.empty:
        ...
        fig_aw.update_layout(
            title=f"올웨더 포트폴리오 자산군 성과 - {selected_aw_period}",
            xaxis_title="날짜",
